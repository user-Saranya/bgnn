...
class GINModelDGL(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim, num_layers=3, dropout=0.0):
        ...
        # First layer
        self.convs.append(
            GINConv(build_mlp(in_dim, hidden_dim), learn_eps=True)
        )
        ...
        for _ in range(num_layers - 2):
            self.convs.append(
                GINConv(build_mlp(hidden_dim, hidden_dim), learn_eps=True)
            )
            self.batch_norms.append(nn.BatchNorm1d(hidden_dim))

        # Final layer
        self.convs.append(
            GINConv(build_mlp(hidden_dim, out_dim), learn_eps=True)
        )
